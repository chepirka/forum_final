...
async function loadSchoolsData() {
    try {
        const response = await fetch('../js/school-data.json');
        if (!response.ok) {
...
async function renderFilteredSchools() {
    ...
    const filteredSchools = schoolsData.filter(school => {
        ...
        const matchesType = typeFilter === '' ||
            (school.type && school.type === typeFilter);

        return matchesSearch && matchesDistrict && matchesProfile && matchesType;
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('typeFilter').addEventListener('change', renderFilteredSchools);
